 ...
 * Reemplaza SpreadsheetApp, Session, LockServihhce con equivalentes de Node.js.
 */
const { SHEETS, readRows, appendRow, setCellValue, deleteRow, nextId, fmtDate, keyOp, addColumnHeaderIfMissing } = require('./sheets');
const nodemailer = require('nodemailer');

/* ============================= HELPERS ==if (!p.ID_Linea) p.ID_Linea = p.ID || p['Operaciones_Activas!A1']; });=========================== */

/**
 * Lee Pagos_Programados y normaliza el nombre de la columna de ID de lÃ­nea.
 * El spreadsheet tiene la columna A como 'ID' en lugar de 'ID_Linea'.
 * Este helper garantiza que siempre estÃ© disponible como p.ID_Linea.
 */
async function readPP() {
  const rows = await readRows(SHEETS.PAGOS_PROG);
  rows.forEach(p => { if (!p.ID_Linea) p.ID_Linea = p.ID || p['Operaciones_Activas!A1']; });
  return rows;
}
...
function fmtGlosaMonto(monto, moneda) {
  // Formato: $470 000,00  o  Â¢150 000 000,00
  const simbolo = moneda === 'USD' ? '$' : 'Â¢';
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#1a2b4a;padding:16px 24px;border-radius:8px 8px 0 0;">
    <h2 style="color:#fff;margin:0;font-size:18px;">â Pago Registrado â Cofersa Deuda Global</h2>
  </div>
  <div style="background:#fff;padding:24px;border:1px solid #e0e0e0;border-radius:0 0 8px 8px;">
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
      <tr><td style="padding:8px 0;color:#666;width:140px;">NÃºmero de Pago</td><td style="padding:8px 0;font-weight:bold;">${pagoId}</td></tr>
      <tr style="background:#f5f5f5;"><td style="padding:8px 4px;color:#666;">LÃ­nea</td><td style="padding:8px 4px;font-weight:bold;">${lineaId}</td></tr>
      <tr><td style="padding:8px 0;color:#666;">Banco</td><td style="padding:8px 0;">${banco}</td></tr>
      <tr style="background:#f5f5f5;"><td style="padding:8px 4px;color:#666;">NÂ° OperaciÃ³n</td><td style="padding:8px 4px;">${numOp}</td></tr>
    ...
    <div style="background:#eef4ff;border:1px solid #b0c8f0;border-radius:6px;padding:16px;margin-top:16px;">
      <p style="margin:0 0 8px 0;font-size:13px;color:#555;font-weight:bold;">ð GLOSA PARA ERP</p>
      <p style="margin:0;font-family:monospace;font-size:14px;color:#1a2b4a;font-weight:bold;letter-spacing:0.3px;">${glosa}</p>
    </div>
    <p style="margin-top:20px;font-size:12px;color:#999;">Generado automÃ¡ticamente por Cofersa Deuda Global Â· ${new Date().toLocaleString('es-CR', { timeZone: 'America/Costa_Rica' })}</p>
  </div>
</div>`;
  ...
  await transporter.sendMail({
    ...
    subject: `Pago ${pagoId} registrado â ${banco} ${numOp} Â· ${fecha}`,
    html,
    text: `Pago registrado\n\nID: ${pagoId}\nLÃ­nea: ${lineaId}\nBanco: ${banco}\nNÂ° Op: ${numOp}\nFecha: ${fecha}\nMonto: ${fmtGlosaMonto(monto, moneda)}\nEstado: ${estado}\n\nGLOSA ERP:\n${glosa}`,
  });

  console.log(`[Email] NotificaciÃ³n de pago ${pagoId} enviada a yugalde@cofersa.cr`);
}
...
async function crearLinea(email, payload) {
  ...
  await appendRow(SHEETS.ACTIVAS, [newId, banco, numOp, 'PrÃ©stamo a Plazo', moneda, monto, tasa || 0, plazoFinal, desembolso, vencimiento, 'â']);
  for (const f of filas) {
    await appendRow(SHEETS.PAGOS_PROG, [newId, f.fecha, f.capital, f.interes, 'Pendiente']);
  }

  await logAudit(user.email, `Nueva lÃ­nea con plan de pagos ${newId} (${plazoFinal} cuotas)`, 'Operaciones', 'Ãxito');
...
async function registrarPago(email, payload) {
  ...
  // Obtener capital e interÃ©s de la cuota si estÃ¡ conciliando
  ...
  // = Ãºltimo pago Pagado/Conciliado de esta lÃ­nea + 1 dÃ­a, o FechaInicio si es el primero
  ...
  // El comprobante detallado lo envÃ­a /api/notificar-pago desde el cliente
  return { id: newId };
}
...
async function reemplazarPlanPagos(email, lineaId, rowsRaw) {
  ...
  await logAudit(user.email, `Plan de pagos reemplazado en ${lineaId}: ${toDelete.length} eliminadas, ${added} nuevas`, 'Operaciones', 'Ãxito');
