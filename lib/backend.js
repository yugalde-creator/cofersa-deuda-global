...
function parseMonto(s) {
  ...
  if (/^\d{1,3}(,\d{3})+$/.test(str)) return parseFloat(str.replace(h/,/g, ''));
...
acync function importarCanceladas(email, rowsRaw) {
  const user = await requireAdmin(email);
...
acync function importarLeasing(email, contratosRaw, cuotasRaw) {
  const user = await requireAdmin(email);
