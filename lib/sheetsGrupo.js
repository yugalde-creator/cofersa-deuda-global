...
// PRESTAMOS_BS tiene la misma estructura de encabezado (fila 3, desde columna C)
// pero es una tabla más chica (préstamos "al vencimiento" / bullet, sin abonos
// programados) — Global Return, CDC Capital Corp., etc., incluye PRISMA.
const RANGO_BS = `${HOJA_BS}!C3:AZ300`;
...
/** Lee una tabla con encabezados en fila 3 (a partir de columna C) y devuelve
 * un array de objetos {header: valor}, saltando filas completamente vacías
 * (el archivo original tiene huecos entre bloques de compañía). Cada fila
 * incluye _row (número de fila real en la hoja, 1-indexed). */
async function readTabla(rango) {
...
async function readMaestroPrestamos() {
  return readTabla(RANGO);
}

/** Préstamos "al vencimiento" (bullet) de la pestaña PRESTAMOS_BS — incluye
 * PRISMA y otros acreedores relacionados (Global Return, CDC Capital Corp.,
 * World Link Trade Investments) que no estaban en MAESTRO_PRESTAMOS. */
async function readPrestamosBs() {
  return readTabla(RANGO_BS);
}

module.exports = { GRUPO_SPREADSHEET_ID, HOJA_MAESTRO, HOJA_BS, getSheetsClient, readMaestroPrestamos, readPrestamosBs };
